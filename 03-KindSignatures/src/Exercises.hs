...
instance Question10 (Monad m)
instance Question10 (Eq m)
instance Question10 (Show m)

-- Monad :: (Type -> Type) -> Constraint is not like Eq, Show :: Type -> Constraint
-- Each of Monad, Eq, Show generate constraints, but Monad m & (Eq m | Show m)
-- makes a kind error over m: Type !~ (Type -> Type).
